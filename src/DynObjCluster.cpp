                                           ...
                                           std::vector<std::vector<int>> &voxel_clusters, const pcl::PointCloud<PointType> &raw_point, std::unordered_set<int> &used_map_set)
{
    ...
    tbb::parallel_for(size_t(0), index_bbox.size(), [&](size_t idx) {
    ...
    tbb::parallel_for(size_t(0), bbox.Center.size(), [&](size_t bbox_i) {
    ...
    tbb::parallel_for(size_t(0), dyn_tag_one_global.size(), [&](size_t idx) {
        dyn_tag[dyn_tag_one_global[idx]] = 1;
    });
    tbb::parallel_for(size_t(0), dyn_tag_zero_global.size(), [&](size_t idx) {
        dyn_tag[dyn_tag_zero_global[idx]] = 0;
    });
    cluster_points.clear();
    cluster_points.points.resize(cluster_points_vec.size());
    tbb::parallel_for(size_t(0), cluster_points_vec.size(), [&](size_t idx) {
    ...
    tbb::parallel_for(size_t(0), true_ground_vec.size(), [&](size_t idx) {
